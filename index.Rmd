...
For predicting proper execution of exercise I fitted and compare two different models:  

* A **CART** model using **rpart** method and **10-fold cross validation** in train function of caret package called **pml_modfit_rpart**,
* A **random forest** model using **rpart** method and **2-fold cross validation** in train function of caret package called **pml_modfit_rf**. For this second model I decided to use just a 2-fold cross validation to reduce computational time in case peer reviewers will decide to re-run my code.
...
Figure show that for predicting the proper execution of exercise magnets located on the dumbell are the most important variables.

## Expectation regarding out of sample error and predictions for test cases

Regarding to out of sample error, I'll expect it to be **greater than in sample error**. Finally I used model to predict values for 20 test cases provided. Following figure show results of my predictions.
